...
export default function Regularity() {
    ...
    console.log('selectedData', selectedData[0]?.check_history[0]?.text?.income_regularity_and_quantum_report.income_sources
    )
    ...
    const paginatedData = selectedData[0]?.check_history[0]?.text?.income_regularity_and_quantum_report.income_sources.slice(
        (currentPage_1 - 1) * itemsPerPage,
        currentPage_1 * itemsPerPage
    );

    const totalPages_1 = Math.ceil(selectedData[0]?.check_history[0]?.text?.income_regularity_and_quantum_report.income_sources?.length / itemsPerPage);
    ...
    return (
        <>
            ...
            <Sidebar title={'Income sources regularity and Quantum Analysis'}>



                <Typography sx={{ fontSize: '14px', color: '#656565', fontWeight: '600', textAlign: 'end', mr: '20px', mt: '20px' }}>Total Score: {paginatedData && paginatedData.map((data) => (
                    data.score
                ))}</Typography>
                ...
                {paginatedData && paginatedData.map((data, index) => (
                    <Box>
                        <Box>
                            ...
                            <>
                                <ul style={{ marginLeft: '10%' }} key={index}>
                                    <li>
                                    ...
                                    <li>
                                        <Typography sx={{ fontSize: '14px', color: '#000', fontWeight: '500', mb: '10px' }}>
                                            <strong>Regularity:</strong> {data.regularity}

                                        </Typography>
                                    </li>
                                    <li>
                                        <Typography sx={{ fontSize: '14px', color: '#000', fontWeight: '500', mb: '10px' }}>
                                            <strong>Quantum:</strong> {data.quantum}
                                    ...
                                    <Table sx={{ border: '1px solid #ddd' }}>
                                        <TableHead sx={{ bgcolor: '#f0f0f4' }}>
                                            <TableRow>
                                                ...
                                                <TableCell sx={{ textAlign: 'start', fontSize: '14px', fontWeight: '600', width: '30%', color: '#656565' }}>Amount</TableCell>
                                            </TableRow>
                                        </TableHead>
                                        <TableBody>
