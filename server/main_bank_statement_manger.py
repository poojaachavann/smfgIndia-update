...
def main(file_path=None, out_dir="assets", uuid=None, process_id=None):
    ...
    converter = PDFToImagesConverter(temp_image_prefix="bank_statement")
    image_paths = converter.convert(file_path, images_dir,)

    # Extract text from images
    extraction_config = ExtractionConfig(model="ai-document-intelligence")
...
```
    ...
    [print("Original Text:", text) for text in texts]
    ...
    for idx, page in enumerate(parsed, start=1):
        ...
        json_path = os.path.join(json_dir, f"bank_statement-page_{idx}.json")
    ...
    combined_json_path = os.path.join(base_path, "bank_statement-combined_manifest.json")
